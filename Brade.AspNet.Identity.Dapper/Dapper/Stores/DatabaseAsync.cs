    ...
    public abstract partial class Database<TDatabase> : IDisposable where TDatabase : Database<TDatabase>, new()
    {
        public partial class Table<T, TId, TPrimaryKeys>
        {
            ...
            public virtual async Task<int?> InsertAsync(dynamic data)
            {
                ...
                // todo get id by provider type for example sqlserverce @@Identity etc...
            ...
            public Task<int> UpdateAsync(TId id, dynamic data)
            {
                ...
                builder.AppendLine(string.Join(",", paramNames.Where(n => !String.Equals(n, "Id", StringComparison.InvariantCultureIgnoreCase)).Select(p => p + "= @" + p)));
